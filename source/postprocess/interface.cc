...
// ------------------------------ Interface -----------------------------
    ...
    template <int dim>
    void Interface<dim>::parse_parameters (ParameterHandler &)
    {}
    ...
    Manager<dim>::parse_parameters (ParameterHandler &prm)
    {
      ...
      if (std::find (postprocessor_names.begin(),
                     ...
                     "all") != postprocessor_names.end())
        {
          ...
          for (typename std::list<typename elaspect::internal::Plugins::PluginList<Interface<dim> >::PluginInfo>::const_iterator
      ...
      // into an order so that dependences run first, and dependents after
      ...
      //   . if all of its dependences are in the list, add it to the end
      //   . if at least one of its dependences are not yet in the list,
      //     skip it
      //
      ...
      // have found a cycle in the dependences and that is clearly a problem
      ...
      while (sorted_names.size() < postprocessors.size())
      {
...

        {
          ...
          for (unsigned int i=0; i<postprocessor_names.size(); ++i, ++pp)
            ...
            {
              // for this postprocessor, check if all of its dependences
              // are already in the list
              const std::list<std::string> deps = (*pp)->required_other_postprocessors();
              bool unmet_dependences = false;
              for (const auto &p : deps)
                if (std::find (sorted_names.begin(),
                               sorted_names.end(),
                               p) == sorted_names.end())
                {
                  unmet_dependences = true;
                  break;
                }

              // if we have unmet dependences, there is nothing we can do
        ...
        if (at_least_one_element_added == false)
        {
          ...
          out << "While sorting postprocessors by their dependences, "
              "elASPECT encountered a cycle in dependences. The following "
