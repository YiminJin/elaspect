...
#include <elaspect/postprocess/topography.h>
...
namespace elaspect
{
  namespace Postprocess
  {
    ...
    {
      const types::boundary_id relevant_boundary = 
        this->get_geometry_model().translate_symbolic_boundary_name_to_id("top");

      const QTrapezoid<dim-1> face_corners;
      FEFaceValues<dim> face_vals(this->get_mapping(),
                                  this->get_fe(),
                                  face_corners,
      ...
      // loop over all surface cells and store the elevation in the stream for output
        ...
        if (cell->is_locally_owned() && cell->at_boundary())
          for (const unsigned int face_no : cell->face_indices())  
          {
            ...
            for (unsigned int corner = 0; corner < face_corners.size(); ++corner)
            {
              ...
              local_max_height = std::max(local_max_height, elevation);
              local_min_height = std::min(local_min_height, elevation);
            }
          }
      ...
      // write results to statistic file
      ...
      // write the topography to file.

      // if this is the first time we get here, set the last output time
      // to (current time - output_interval). this makes sure we always
      // produce data during the first time step
      ...
      prm.enter_subsection("Postprocess");
      {
        prm.enter_subsection("Topography");
        {
          prm.declare_entry("Output to file", "false",
                            Patterns::Bool(),
                            "Whether or not to write topography to a text file named "
...
namespace elaspect
{
  namespace Postprocess
  {
    ELASPECT_REGISTER_POSTPROCESSOR(Topography,
                                "topography",
