  ...
  namespace internal
  {
    namespace Assembly
    {
      namespace Scratch
      {
        ...
        MechanicalSystem (const FiniteElement<dim> &finite_element,
          ...
          fe_values (mapping, finite_element, quadrature, update_flags),
          fe_face_values (mapping, finite_element, face_quadrature, face_update_flags),
        ...
        template <int dim>
        ThermoSystem<dim>::
        ThermoSystem (const FiniteElement<dim> &finite_element,
                       const Mapping<dim>       &mapping,
                       const Quadrature<dim>    &quadrature,
                       const Quadrature<dim-1>  &face_quadrature,
                       const UpdateFlags         update_flags,
          ...
          laplacian_phi_T(T_dofs_per_cell, numbers::signaling_nan<double>()),
        ...
        QPDSystem<dim>::QPDSystem(const FiniteElement<dim> &fe,
          ...
          face_phi_field(field_dofs_per_cell, numbers::signaling_nan<double>()),
          neighbor_face_phi_field(field_dofs_per_cell, numbers::signaling_nan<double>()),
      ...
      namespace CopyData
      {
        ...
        template <int dim>
        ThermoSystem<dim>::ThermoSystem (const ThermoSystem<dim> &data)
          :
          local_matrix (data.local_matrix),
          local_rhs (data.local_rhs),
          local_dof_indices (data.local_dof_indices)
        ...
        QPDSystem<dim>::QPDSystem(const unsigned int field_dofs_per_cell,
          ...
          local_matrices_int_ext((use_ALE_method
                                  ?
          ...
          local_matrices_ext_int((use_ALE_method
                                  ?
                                  GeometryInfo<dim>::max_children_per_face * GeometryInfo<dim>::faces_per_cell
                                  :
                                  0),
                                 FullMatrix<double>(field_dofs_per_cell, field_dofs_per_cell)),
          local_matrices_ext_ext((use_ALE_method
                                  ?
                                  GeometryInfo<dim>::max_children_per_face * GeometryInfo<dim>::faces_per_cell
                                  :
                                  0),
          ...
          assembled_matrices((use_ALE_method
                              ?
                              GeometryInfo<dim>::max_children_per_face * GeometryInfo<dim>::faces_per_cell
                              :
                              0), false),
          ...
          neighbor_dof_indices((use_ALE_method
                                ?
                                GeometryInfo<dim>::max_children_per_face * GeometryInfo<dim>::faces_per_cell
                                :
