...
// -------------------------------- Deal with registering free surface models and automating
// -------------------------------- their setup and selection at run time
      ...
      create_free_surface_model(ParameterHandler &prm)
      {
        ...
        AssertThrow(model_name != "unspecified",
                    ExcMessage("You need to select a free surface model "
                               "(`set Model name' in `subsection Mesh deformation::Free surface')."));
        return std::get<dim>(registered_plugins).create_plugin(model_name, 
                                                               "Mesh deformation::Free surface::Model name");
      }
        ...
        prm.enter_subsection("Mesh deformation");
{
          prm.enter_subsection("Free surface");
          {
            ...
            prm.declare_entry("Model name", "simple",
...
namespace elaspect
{
  namespace internal
  {
    ...
    {
      template <>
      std::list<internal::Plugins::PluginList<MeshDeformation::FreeSurface::Interface<2> >::PluginInfo> *
      internal::Plugins::PluginList<MeshDeformation::FreeSurface::Interface<2> >::plugins = nullptr;

      template <>
      std::list<internal::Plugins::PluginList<MeshDeformation::FreeSurface::Interface<3> >::PluginInfo> *
      internal::Plugins::PluginList<MeshDeformation::FreeSurface::Interface<3> >::plugins = nullptr;
    }
  }
...
#define INSTANTIATE(dim) \
      ...
      template \
      void \
      register_free_surface_model<dim>(const std::string &, \
                                       const std::string &, \
      ...
      template \
      Interface<dim> * \
      create_free_surface_model<dim>(ParameterHandler &); \
      \
      template  \
      void \
