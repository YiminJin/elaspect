...
#include <elaspect/mesh_refinement/composition_gradient.h>
  ...
  namespace MeshRefinement
  {
    ...
    CompositionGradient<dim>::execute(Vector<float> &indicators) const
    {
      AssertThrow(this->n_compositional_fields() >= 1,
                  ExcMessage("Refinement criterion 'composition' cannot be used when "
                             "no compositional fields are active!"));

      indicators = 0;
      // create a vector with the requisite ghost elements
      // and use it for estimating the gradients of compositional field
      TrilinosWrappers::MPI::BlockVector vec(this->introspection().index_sets.system_partitioning,
    ...
    CompositionGradient<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Mesh refinement");
      {
        prm.enter_subsection("Composition gradient");
        {
          composition_scaling_factors = Utilities::string_to_double(
...
namespace elaspect
{
  namespace MeshRefinement
  {
    ELASPECT_REGISTER_MESH_REFINEMENT_CRITERION(CompositionGradient,
                                            ...
                                            "\n\n"
                                            "The current criterion does not compute the gradient "
                                            "at quadrature points on each cell, but by a finite "
                                            "difference approximation between the centers of cells, "
                                            "which is particularly suitable for gradient estimation "
                                            "of discontinuous finite elements.")
  }
}
